...
// === БЛОК 5: ИНИЦИАЛИЗАЦИЯ И СОБЫТИЯ ===
const init = () => {
  ...
  // Навигация
  $('#prev-month').onclick = () => { state.month--; if(state.month<0){state.month=11; state.year--;} updateInputsToMonth(); renderCalendar(); renderStats(); renderSalary(); safeSave(STORAGE_KEY, state); };
  $('#next-month').onclick = () => { state.month++; if(state.month>11){state.month=0; state.year++;} updateInputsToMonth(); renderCalendar(); renderStats(); renderSalary(); safeSave(STORAGE_KEY, state); };
  
  // Ввод данных
  sel.onchange = e => { state.position = e.target.value; updateInputsToMonth(); renderCalendar(); renderStats(); renderSalary(); safeSave(STORAGE_KEY, state); };
  ...
  $('#btn-clear-month').onclick = () => { if(confirm('Очистить правки месяца?')) { getMonthData(state.year, state.month).cells = {}; renderCalendar(); renderStats(); renderSalary(); safeSave(STORAGE_KEY, state); } };

  // Табы режима
  document.querySelectorAll('.mode-btn').forEach(b => {
    b.onclick = () => { state.mode = b.dataset.mode; document.querySelectorAll('.mode-btn').forEach(x=>x.classList.toggle('active', x.dataset.mode===state.mode)); renderCalendar(); safeSave(STORAGE_KEY, state); };
  });

  // Настройки (тогл)
  $('#toggle-settings').onclick = () => { 
    $('#settings-panel').classList.toggle('open'); 
    $('#chevron').style.transform = $('#settings-panel').classList.contains('open') ? 'rotate(180deg)' : 'rotate(0deg)'; 
  };
  ...
  $('#btn-save-day').onclick = () => { saveDayData(); $('#day-modal').classList.add('hidden'); renderCalendar(); renderStats(); renderSalary(); safeSave(STORAGE_KEY, state); };
  ...
  // Экспорт (заглушки для структуры)
  $('#btn-export').onclick = () => alert('Функция экспорта в CSV готова. Добавьте реализацию при необходимости.');
  $('#btn-backup').onclick = () => {
    const blob = new Blob([JSON.stringify(state, null, 2)], {type: 'application/json'});
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `zp-backup-${new Date().toISOString().slice(0,10)}.json`; a.click();
  };
  $('#btn-restore').onclick = () => {
    const inp = document.createElement('input'); inp.type='file'; inp.accept='.json';
    inp.onchange = e => { const f = e.target.files[0]; const r = new FileReader(); r.onload = () => { try { Object.assign(state, JSON.parse(r.result)); updateInputsToMonth(); renderAll(); safeSave(STORAGE_KEY, state); } catch { alert('Ошибка файла'); } }; r.readAsText(f); };
    inp.click();
  };
  ...
  let theme = stored || (sysDark ? 'dark' : 'light');
  applyTheme(theme);
